...
public:
    ...
    int load(void* input, size_t size_input = -1) {
        ...
        for (int i = 0; i < entry_count; i++) {
            entry_buffer.part                   = storage.read_int<std::uint64_t>(std::endian::little);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.panel_id               = storage.read_str(0, ptr_buffer64 - 8);
            entry_buffer.page                   = storage.read_int<std::uint64_t>(std::endian::little);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.boss_panel             = storage.read_str(0, ptr_buffer64 - 8);
            storage.change_pos(16); // 2 unk
            entry_buffer.gold_reward            = storage.read_int<std::uint32_t>(std::endian::little);
            entry_buffer.type                   = (Entry::Panel_Type)storage.read_int<std::uint32_t>(std::endian::little);

            // Adjacent panels
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.adjacent_panels.up     = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.adjacent_panels.down   = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.adjacent_panels.left   = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.adjacent_panels.right  = storage.read_str(0, ptr_buffer64 - 8);

            entry_buffer.display_difficulty     = storage.read_int<std::uint32_t>(std::endian::little);
            entry_buffer.cpu_level              = storage.read_int<std::uint32_t>(std::endian::little);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.stage_id               = storage.read_str(0, ptr_buffer64 - 8);
            storage.change_pos(8); // unk
            entry_buffer.first_speaker          = storage.read_int<std::uint64_t>(std::endian::little);

            // Player
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.player.id              = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.player.assist_id       = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.player.btlst_id        = storage.read_str(0, ptr_buffer64 - 8);
            storage.change_pos(8); // unk
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.player.btlwin_id       = storage.read_str(0, ptr_buffer64 - 8);

            // Enemy
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.enemy.id               = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.enemy.assist_id        = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.enemy.btlst_id         = storage.read_str(0, ptr_buffer64 - 8);
            storage.change_pos(8); // unk
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.enemy.btlwin_id        = storage.read_str(0, ptr_buffer64 - 8);

            // Special rules
            entry_buffer.special_rule[0]    = storage.read_int<std::int32_t>(std::endian::little);
            entry_buffer.special_rule[1]    = storage.read_int<std::int32_t>(std::endian::little);
            entry_buffer.special_rule[2]    = storage.read_int<std::int32_t>(std::endian::little);
            entry_buffer.special_rule[3]    = storage.read_int<std::int32_t>(std::endian::little);

            // Secret missions
            for (int i = 0; i < 4; i++) {
                entry_buffer.secret_mission[i].condition    = storage.read_int<std::int32_t>(std::endian::little);
                entry_buffer.secret_mission[i].unk          = storage.read_int<std::int32_t>(std::endian::little);
                ptr_buffer64                                = storage.read_int<std::uint64_t>(std::endian::little);
                entry_buffer.secret_mission[i].reward_id    = storage.read_str(0, ptr_buffer64 - 8);
                entry_buffer.secret_mission[i].gold_reward  = storage.read_int<std::uint64_t>(std::endian::little);
            }

            entry_buffer.unk0                                   = storage.read_int<std::uint32_t>(std::endian::little);
            entry_buffer.unk1                                   = storage.read_int<std::uint32_t>(std::endian::little);
            entry_buffer.unk2                                   = storage.read_int<std::uint32_t>(std::endian::little);

            entry_buffer.index                                  = storage.read_int<std::uint32_t>(std::endian::little);
    ...
    std::uint64_t* write_to_bin() {
        ...
        for (auto& [key, entry] : entries) {
            storage.write_int<std::uint64_t>(entry.part, std::endian::little);
            write_offset_str(entry.panel_id);
            storage.write_int<std::uint64_t>(entry.page, std::endian::little);
            write_offset_str(entry.boss_panel);
            storage.write_int<std::uint64_t>(0, std::endian::little); // unk
            storage.write_int<std::uint64_t>(0, std::endian::little); // unk
            storage.write_int<std::uint32_t>(entry.gold_reward, std::endian::little);
            storage.write_int<std::uint32_t>((int)entry.type, std::endian::little);
            ...
            storage.write_int<std::uint32_t>(entry.display_difficulty, std::endian::little);
            storage.write_int<std::uint32_t>(entry.cpu_level, std::endian::little);
            write_offset_str(entry.stage_id);
            storage.write_int<std::uint64_t>(1, std::endian::little); // unk
            storage.write_int<std::uint64_t>(entry.first_speaker, std::endian::little);
            ...
            for (int i = 0; i < 4; i++) {
              storage.write_int<std::uint32_t>((int)entry.special_rule[i], std::endian::little);
            }

            for (int i = 0; i < 4; i++) {
                storage.write_int<std::uint32_t>((int)entry.secret_mission[i].condition, std::endian::little);
                storage.write_int<std::int32_t>(entry.secret_mission[i].unk, std::endian::little);
                write_offset_str(entry.secret_mission[i].reward_id);
                storage.write_int<std::uint64_t>(entry.secret_mission[i].gold_reward, std::endian::little);
            }

            storage.write_int<std::uint32_t>(entry.unk0, std::endian::little);
            storage.write_int<std::uint32_t>(entry.unk1, std::endian::little);
            storage.write_int<std::uint32_t>(entry.unk2, std::endian::little);

            storage.write_int<std::uint32_t>(entry.index, std::endian::little);
        }
