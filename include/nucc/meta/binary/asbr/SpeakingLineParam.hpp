...
public:
    ...
    int load(void* input, size_t size_input = -1) {
        ...
        for (int i = 0; i < entry_count; i++) {
            entry_buffer.interaction_type       = (Entry::Interaction_Type)storage.read_int<std::uint32_t>(std::endian::little);
            entry_buffer.is_animation           = storage.read_int<std::uint32_t>(std::endian::little);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.character_1.id         = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.character_2.id         = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.character_1.audio   = storage.read_str(0, ptr_buffer64 - 8);
            ptr_buffer64                        = storage.read_int<std::uint64_t>(std::endian::little);
            entry_buffer.character_2.audio   = storage.read_str(0, ptr_buffer64 - 8);
    ...
    std::uint64_t* write_to_bin() {
        ...
        for (auto& [key, entry] : entries) {
            storage.write_int<std::uint32_t>((int)entry.interaction_type, std::endian::little);
            storage.write_int<std::uint32_t>((int)entry.is_animation, std::endian::little);
        ...
        for (auto& [key, entry] : entries) {
            storage.write_str(entry.character_1.id);
            storage.align_by(8);
            storage.write_str(entry.character_2.id);
            storage.align_by(8);
            storage.write_str(entry.character_1.audio);
            storage.align_by(8);
            storage.write_str(entry.character_2.audio);
            storage.align_by(8);
        }
